...
\includegraphics[width=\columnwidth]{figures/convergence_gmm_v2.pdf}
  \caption{Log joint $\log p(x, z)$ as a function of number of sweeps $K$ for a GMM instance. The APG sampler and Gibbs sampler have a sampling budget of $L=100$ samples per sweep, while BPS has a sampling budgets of $10,000$ per sweep. HMC-RWS has a budget of $100$ samples per leapfrog per step sweep. To achive results comparable to AGP and Gibbs samplers, HMC-RWS needs a $10$ times larger sampling budget while BPS under-performs even with a $100$ times larger sampling budget.}
...
\begin{table*}[t!]
\centering
\caption{Averaged log joint $\log p_\q (x, z)$ of the test corpora for RWS, BPS, HMC-RWS, and Gibbs Sampling for different numbers of sweeps $K$.
