...
To evaluate the performance on non-conjugate models we trained a deep generative mixture model and an unsupervised tracking model and compare APG samplers to reweighted wake-sleep (RWS) and a HMC-augmented RWS baseline (HMC-RWS).
...
% [labelwidth=0.5em,labelsep=0.5em,leftmargin=1.0em,topsep=0em,label={\tiny\raisebox{0.7ex}{\textbullet}}]
    \item We develop APG samplers, a novel amortized inference method based on adaptive importance sampling
    ...
    \item APG samplers can be used to train structured models with 100s of variables for state-of-the-art inference problems in an unsupervised manner 
    \item Experiments show substantial improvement during test-time inference compared to existing inference methods
\end{enumerate}
