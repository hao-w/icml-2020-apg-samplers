...
% TA: the paragraph above is a potential replacement of the paragraph below
...
% Training the Gibbs conditionals using importance sampling is slow due to high variance gradient estimation and if during test time these conditionals do not match the posterior conditionals exactly, the resulting Markov Chain will be a poor approximation to the full posterior.
...
To address both of these challenges, we use the Gibbs conditionals as proposals of a sequential Monte Carlo (SMC) sampler~\citep{delmoral2006sequential}.
% To solve this, we will use the proposals to define a sequential Monte Carlo (SMC) sampler \citep{delmoral2006sequential}.
%SMC methods reduce the variance of importance weights by decomposing a high-dimensional sampling problem into a sequence of lower-dimensional problems \cite{}. 
