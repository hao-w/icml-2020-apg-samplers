...
\begin{figure}[t!]
  \centering
  \includegraphics[width=\columnwidth]{figures/gmm_samples.pdf}
  \vspace*{-1mm}
  \label{samples-gmm}
  \caption{Single samples in GMM . The left column shows one randomly-drawn test dataset, The subsequent columns show inference results by RWS, followed by results after different number of APG updates.}
\end{figure}
...
\includegraphics[width=\columnwidth]{figures/gmm_blocks_comparison.pdf}
  \caption{Analysis with different block strategies under the same computation budget.}
  \label{fig:convergence-gmm}
\end{figure}
...
In both GMM, we compute the log-joint distribution $\log p_\q(\x, z)$ (see Table \ref{table-both-mm}) as a function of sweep iteration to measure the convergence and the effective sample size (see Table \ref{table-both-mm}) to assess proposal quality
\begin{align}
\label{ess-eq}
    \frac{\text{ESS}}{L} 
    = 
...
\toprule
     & RWS & HMC1x & HMC5x & HMC10x & BPS & \multicolumn{3}{c}{APG} & GIBBS \\
     & & K=20 & K=20 & K=20 & K=20 & K=5 & K=10 & K=20 & K=20 \\
    \midrule
    GMM & -765.13 & -659.45 & -542.01 & -467.3 & -668.89 & -481.65 & -455.66 & -446.91 & -457.33 \\
    DGMM & -3172.0 & -2184.9 & -2184.2 & -2184.0 & -4204.5 & -2050.4 & -2040.2 & -2035.1 & -- \\
    % BMNIST & -- & --& -- & -- & -- & --\\
    \bottomrule
\end{tabular}
\end{table*}
