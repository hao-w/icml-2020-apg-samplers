...
We consider a \emph{sweep} of conditional proposals as
\begin{align}
    \label{eq:approx-gibbs-kernel}
    ...
    ,
\end{align}
where $\z_{\prec b} = \{z_i \mid i < b\}$ and $\z_{\succ b} = \{z_i \mid i > b\}$. We will show that at 

$p_\q(\z_b \,|\, x, z+{-b})$ will leave the posterior invariant as long as each single proposal leaves the poste
...
\begin{align*}
    ...
    \: 
    p_\q(\z^{k-1} \mid x)
    \prod_{b=1}^b p_\q(\z^k_b \mid \x, \z^{k}_{\prec b}, \z^{k-1}_{\succ b})
    \\
    &=
    \int 
    dz^{k-1}_{\preceq b} 
