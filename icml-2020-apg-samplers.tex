...
\includegraphics[width=\columnwidth]{figures/convergence_gmm.pdf}
  \caption{Convergence of $\log p_\theta(x, z)$ in GMM (the test sets in Figure~\ref{samples-mixture}).}
  \label{fig:convergence-gmm}
\end{figure}

\begin{figure}[t!]
\centering
\includegraphics[width=\columnwidth]{figures/kl_training_gmm.pdf}
  \caption{Objective $\mathrm{KL} (p_\q(z_b | x, z_{-b} || q_\f(z_b | x, z_{-b})))$ as a function of wall-clock time for GMM experiment. Comparison among various number of sweeps given the same computation budget.}
  \label{fig:convergence-gmm}
\end{figure}
...
\begin{table}[t!]
\centering
\caption{$\log p_\theta(x, z)$ (the test sets in Figure~\ref{samples-mixture}) with different combinations of number of APG sweeps $K$ and number of particles $L$. The main diagonal elements (highlighted by green boxes) are results under same computation budget $K \cdot L = 1000$.}
\begin{tabular}{ccccc}
\toprule
K = 1 & K = 5 & K = 10 & K = 20 & K = 50 \\
L = 1000 & L = 200 & L = 100 & L = 50 & L = 20 \\
\midrule
    -722.2 & -523.6 & -453.7 & -426.9 & -425.5\\
\bottomrule
\end{tabular}
\end{table}
% \begin{figure*}[t!]
