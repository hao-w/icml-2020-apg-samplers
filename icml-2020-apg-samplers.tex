...
To understand how approximate Gibbs proposals can be used within a SMC sampler, we will first define a sequential importance sampler (SIS), which decomposes the importance weight into a sequence of \emph{incremental} weights. In general, SIS considers a sequence of unnormalized target densities $\gamma^1(z^1), \gamma^2(z^{1:2}), \dots, \gamma^K(z^{1:K})$. If we now consider an initial proposal $q^1(z^1)$, along with a sequence of conditional proposals $q^k(z^k \mid z^{1:k-1})$, then we can recursively construct a sequence of weights $w^k = v^k w^{k-1}$ by assuming $w^1 = \gamma^1(z^1) / q^1(z^1)$ and defining the incremental weight
\begin{align*}
    v^k 
    &=
...
%As a means of assessing how much the learned proposals improved performance, we implement the bootstrapped population sampler (BPS), which employs the exact APG sampling scheme but proposes global (instance-level) variables from the prior $p(z)$. Since sampling from the prior is less likely to generate high-quality samples, we perform BPS with many more particles for the comparison.
...
\includegraphics[width=\columnwidth]{figures/convergence_gmm_v2.pdf}
  \caption{$\log p_\theta(x, z)$ as a function of number of sweeps for a GMM test instance. We perform APG sampling and Gibbs sampling using $L=100$ particles, BPS using $L=10,000$ particles (i.e.~$100\times$ the budget), HMC-RWS using $L=100$ particles and $1, 5, 10$ leapfrog steps (i.e.~$1, 5, 10\times$ the budget), respectively. To achive results comparable to APG and Gibbs samplers, HMC-RWS needs a $10\times$ larger computation budget, while BPS underperforms even with a $100\times$ budget.}
...
We compare the APG samplers with true Gibbs samplers and the baselines in section~\ref{sec:baselines}. Figure~\ref{fig:convergence-gmm} shows that the APG sampler quickly converges to the $p_\q(x, z)$ achieved  the true Gibbs sampler; it outperforms HMC even when we run it with 10 times the computation budget; and learned proposals substantially improve on inference since BPS does not converge even with 100 times the budget.
