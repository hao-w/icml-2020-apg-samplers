...
\begin{align}
    ...
    .
\end{align}
This construction is valid for \emph{any} proposal kernel $q(z^k \mid z^{k-1})$ and reverse kernel $r(z^{k-1} \mid z^{k})$. For a given \emph{forward kernel} The \emph{optimal} reverse kernel for a proposal is
\begin{align*}
    r(z^{k-1} \mid  z^k)
    =   
    \frac{p_\q(x,z^{k-1})}
...
We assess additionally assess sample quality using the effective sample size
\vspace{-0.5em}
\begin{align*}
% \label{ess-eq}
    \frac{\text{ESS}}{L} 
...
\begin{table}[t!]
    \centering
    \caption{Mean squared error (MSE) between test instance and its reconstruction across corpora (each has 5000 video instances) with different number of time steps $T$ and digits $D$. All testings are performed using fixed budget $KL=1000$. APG sampler substantially improves the reconstruction (by improving sample quality of variables) from RWS encoder, compared with HMC-RWS.}
