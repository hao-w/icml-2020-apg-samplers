...
\begin{align}
    ...
    .
\end{align}
This construction is valid for any \emph{proposal kernel}  $q(z^k \mid z^{k-1})$ and reverse kernel $r(z^{k-1} \mid z^{k})$. For a given proposal the \emph{optimal} reverse kernel is given by
\begin{align*}
    r(z^{k-1} \mid  z^k)
    =   
    \frac{p_\q(x,z^{k-1})}
...
(2) We run bootstrapped population sampling (BPS) where we employ the APG sampling scheme but propose $\mu, \tau$ from the prior $p(\mu, \tau)$. This shows how much we improve by learning the proposals under the same sampling scheme. (3) We perform Hamiltonian Monte Carlo (HMC) method for continuous variables $\mu, \tau$ by marginalizing the discrete variables $c$, and Gibbs updates $p(c | x, \mu, \tau)$ for discrete variables. Figure~\ref{fig:convergence-gmm} shows that APG sampler outperforms even when we run HMC with 10 times the computation budget; And learned proposals substantially improves on inference since BPS does not converge even with 100 times budget.
\begin{figure}[t!]
\centering
\includegraphics[width=\columnwidth]{figures/budget_gmm.pdf}
  \caption{$\log p(x, z)$ (left) and ESS (right) as a function of number of sweeps for APG samplers with different block update strategies and same overall sampling budget $K \cdot L = 1000$. A higher ESS can be is achieved by performing more sweeps with fewer samples.}
...
\begin{table}[t!]
    \centering
    \caption{Mean-squared error (MSE) between test instance and its reconstruction across corpora (each has 5000 video instances) with different number of time steps $T$ and digits $D$. All tests are performed using fixed budget $KL=1000$. APG samplers achieve substantial improvements relative to RWS and HMC-RWS.}
    \vspace{0.5em}
