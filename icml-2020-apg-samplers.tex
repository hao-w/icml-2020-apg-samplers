...
These models are trained by minimizing the stochastic lower bound in Equation~\ref{eq:elbo} (ELBO) on the log marginal likelihood which is equivalent to minimizing the exclusive KL divergence $\mathrm{KL}(q_\f(z | x) || p_\q(z | x))$. If the distribution specified by the encoder is reparameterizable we can derive a low variance path-wise gradient estimator.
\begin{align}
    \label{eq:elbo}
    \mathcal{L} (\f)
    &
...
When we also require that each single block update $q_\f(z'_b \mid x, z_{-b})$ leaves the target density invariant, we see that a block update must equal the exact conditional posterior, $q_\f(z'_b \mid x, z_{-b}) = p_\q(z'_b \mid x, z_{-b})$.
...
\end{align*}
Here $\mu_i$ is center of the $i$th ring. Given a cluster assignment $c_n$ and an embedding $\alpha_n$ which goes into a decoder $g_\q$, we sample a data point $x_n$ with 2D Gaussian noise. 

We employ a variational distribution that updates global variables $q_\f(\mu | x, c, \alpha)$ and local variables $q_\f(c, \alpha | x, \mu)$. Using APG sampler, we train our model on 20,000 DMM instances, each containing $I = 4$ clusters and $N=200$ points, with $K=8$ sweeps, $L=10$ particles, 20 instances per batch, learning rate $\mathrm{lr} = 10^{-4}$, and $3\times10^5$ gradient steps. (see Appendix \ref{appendix:architecture} for architecture).
...
Finally, we apply the APG sampler to an unsupervised tracking model.


that is trained with video instances $T=10$ time steps and $D=3$ bouncing MNIST digits, and evaluate it with test instances containing $T=100$ time steps and $D=5$ digits.
