...
In recent years we have seen applications of amortized variational methods to inference in hierarchical domains, such as object detection \cite{eslami2016attend}, modeling of user reviews \cite{esmaeili2019structured}, or object tracking \cite{kosiorek2018sequential}. These approaches build on the framework of variational autoencoders (VAEs) \cite{kingma2013auto-encoding, rezende2014stochastic} to train structured deep generative models. However, scaling up these approaches to more complex domains still poses significant challenges. Whereas it is easy to train VAEs on large corpora of data, it is not easy to train structured encoders for models with a large number of (correlated) variables at the instance level. 
...
To evaluate the performance on non-conjugate models we trained a deep generative mixture model and an unsupervised tracking model and compare APG samplers to reweighted wake-sleep (RWS)  and a HMC-augmented variational inference baseline (HMC-VI).
Both models are representative of current state-of-the art unsupervised inference problems.
In all experiments APG samplers where able to capture the hierarchical characteristics of the problem and outperformed existing methods in terms of training efficiency and inference accuracy.
...
% [labelwidth=0.5em,labelsep=0.5em,leftmargin=1.0em,topsep=0em,label={\tiny\raisebox{0.7ex}{\textbullet}}]
    ...
    \item We used the APG sampler framwork to train structured models with 100s of variables for state-of-the-art inference problems in an unsupervided manner 
...
\begin{align*}
    ...
    .
\end{align*}
Reweighted wake-sleep (RWS) methods \cite{bornschein2014reweighted} minimize a stochastic upper bound (EUBO), which is equivalent to minimizing an inclusive KL divergence
\begin{align*}
    \mathcal{U} (\f)
    &
    = 
...
\label{eq:grad-phi-rws}
    ...
    .
\end{align}
This gradient estimator has a number of advantages over the estimator used in standard VAEs \cite{le2019revisiting}. Firstly it does not rely on reparameterization, which is required by VAE gradient estimators.
% VAE objectives rely on reparameterization to compute gradient estimates, which is not required by estimator in Equation~\ref{eq:grad-phi-rws}.
For discrete variables, reparameterization is not possible. This means that we need to compute likelihood-ratio estimators (also known as REINFORCE-style estimators \cite{williams1992simple}), which can have very high variance. A range of approaches for variance reduction have been put forward, including continuous relaxations that are amenable to reparameterization \cite{maddison2017concrete,jang2017categorical}, credit assignment techniques (see \cite{weber2019credit} for a review), and other control variates \cite{mnih2016variational,tucker2017rebar,grathwohl2018backpropagation}. 

The estimator in Equation~\ref{eq:grad-phi-rws} sidesteps the need for these variance reduction techniques. To compute this gradient, we only require that the proposal \emph{density} is differentiable, whereas reparameterized estimators require that the \emph{sample} itself is differentiable. This is a milder condition, that holds for most distributions of interest, including those over discrete variables. Moreover, since this estimator minimizes the inclusive KL divergence, and not the exclusive KL divergence, there is smaller risk of learning a proposal that collapses to a single mode of a multi-modal posterior \cite{le2019revisiting}.
...
\begin{figure*}[t!]
  ...
  \vspace{-1ex}
  \end{subfigure}
  \caption{Single samples in (\textbf{a}) GMM instances with $N=100$ data points and (\textbf{b}) DMM instances with $N=600$ data points. The left columns show the data sets, the subsequent columns show inference results by RWS, followed by results after $K$ APG updates. In addition the right column in (\textbf{b}) DMM  shows reconstructions from the deep generative model.}
...
\includegraphics[width=\columnwidth]{figures/convergence_gmm_v2.pdf}
  \caption{Convergence of log joint $\log p(x, z)$ as a function of number of sweeps in one GMM instance. Here APG sampler and Gibbs sampler are perform using the same computation budget, while BPS uses 100 times as many particles as them, and HMC method are run with 1, 5, 10 times as many computation budget (i.e.~1, 5, 10 leapfrog steps) as the first two methods.}
...
\includegraphics[width=\columnwidth]{figures/budget_gmm.pdf}
  \caption{Comparison APG performance with 2 different block update strategies. $\log p(x, z)$ (left) and ESS (right) are computed as a function of number of sweeps under the same computation budget $K \cdot L = 1000$. Better performance is reached by doing more number of sweeps.}
...
\begin{table*}[t!]
\centering
\caption{Average log joint $\log p_\q (x, z)$ over testing corpus using APG sampler. In GMM, testing corpus contains $20000$ individual instances, each of which has $N=100$ data points; In DMM, testing corpus contains $20000$ individual instances , each of which has $N=600$ data points; In bouncing MNIST, testing corpus contains $10000$ instances, each of which has $T=100$ time steps and $D=5$ digits. We run RWS method as baseline. In addition, we augment RWS with HMC method, BPS. We run HMC with 1, 5, 10 times as many computation budget (i.e.~1, 5, 10 leapfrog steps) as APG samplers.}
...
\begin{table}[t!]
    \centering
    \caption{Mean squared error between reconstruction and data on different testing corpuses, each of which contains $5000$ video instances. The APG sampler can not only outperform the RWS and HMC method, but scales up to $T=100$ time steps at testing, while the training data only has $T=10$ time steps.}
    \begin{tabular}{ccccc}
    \toprule
        & RWS & HMC10x & APG & APG \\
        &  &  K=20 & K=10 & K=20 \\
