...
%   \State{$W_0 \leftarrow 0$}
    ...
    \State{Compute inclusive prefix sum of  $w_1,  \ldots, w_L$ to obtain $W_1, \ldots , W_L$}
    \For {$i = 1$ \textbf{to} $L$}
        \State $a^i = \left\lfloor \frac{LW_i}{W_L} - u \right\rfloor + 1$ \Comment{Index Selection} 
        %   \State{Compute adjacent differences between $K'_1, \ldots, K'_L$ to obtain $k_1, \ldots, k_L$}
        \State Set $ z'^{\:i} = z^{a^i}$
        \State Set $ w'^{\:i} = \frac{1}{L} \sum_{l = 1}^L w^l$ \Comment{Re-Weigh}
    \EndFor
    \State \textbf{Return} $z'^{\:1:L}, w'^{\:1:L}$
