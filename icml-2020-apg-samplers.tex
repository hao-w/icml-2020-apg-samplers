...
In the absence of supervision, uncovering structure from data requires inductive biases. Deep generative models provide a path towards incorporating biases in the form of priors that mirror the structure of the problem domain. By %learning a model in which we 
parameterizing conditional distributions with neural networks, we hope to learn models that use corpus-level characteristics (e.g.~the appearance of objects and the motion dynamics) to make data-efficient inferences about instance-level variables (e.g.~object appearance and positions).
...
To evaluate the performance on non-conjugate models we trained a deep generative mixture model and an unsupervised tracking model and compare APG samplers to reweighted wake-sleep (RWS) and an HMC-augmented RWS baseline (HMC-RWS).
Both models are representative of current state-of-the art unsupervised inference problems.
In all experiments APG samplers were able to capture the hierarchical characteristics of the problem and outperformed existing methods in terms of training efficiency and inference accuracy.
...
% [labelwidth=0.5em,labelsep=0.5em,leftmargin=1.0em,topsep=0em,label={\tiny\raisebox{0.7ex}{\textbullet}}]
    \item We develop APG samplers, a novel amortized inference methods based on adaptive importance sampling
    \item We present a novel parameterization using neural suffi- cient statistics that accounts for the size of the input data
    \item We used the APG sampler framwork to train structured models with 100s of variables for state-of-the-art inference problems in an unsupervised manner 
    \item Experiments show substantial improvment during test-time inference compared to existing inference methods
\end{enumerate}
...
In probabilistic machine learning, we define generative models in terms of a joint distribution $p_\q(\x, \z)$ over data $\x$ and latent variables $\z$. Given a model specification, we are interested in reasoning about the posterior distribution $p_\q(\z \mid \x)$ conditioned on data $\x \sim p^\textsc{data}(\x)$ that is sampled from an (unknown) data distribution. Amortized variational inference methods approximate the posterior $p_\q(\z \mid \x)$ with a distribution $q_\f(\z \mid \x)$ in some tractable variational family.
...
A most commonly used class of deep probabilistic models are variational autoencoders (VAEs) \cite{kingma2013auto-encoding, rezende2014stochastic}. In VAEs, generative model combines an unstructured prior (e.g.~a spherical Gaussian) with a likelihood that is parameterized by a neural network, known as a decoder. The inference network, known as an encoder, maps input data (e.g.~an image or sentence) onto an embedding vector, also known as the latent code. VAEs minimize the exclusive KL divergence $\mathrm{KL}(q_\f(z | x) || p_\q(z | x))$ by maximizing a stochastic evidence lower bound
\begin{align*}
    \mathcal{L} (\f)
    &
    = 
...
\subsection{Reweighted Wake-Sleep Methods}
\label{background-rws}
Reweighted wake-sleep (RWS) methods \cite{bornschein2014reweighted} minimize a stochastic upper bound, which is equivalent to minimizing an inclusive KL divergence
\begin{align*}
    \mathcal{U} (\f)
    &
    = 
    ...
    .
\end{align*}
To compute the gradient w.r.t. $\f$, reweighted wake-sleep methods define self-normalized gradient estimator by sampling from an encoder $z \sim q_\f(z \mid x)$ and compute weights $w = p_\q(x, z) / q_\f(z \mid x)$
\begin{align}
...
The estimator in Equation~\ref{eq:grad-self-normalized} is similar to the gradient estimator (in Equation~\ref{eq:grad-phi-rws}) in reweighted wake-sleep methods, which also minimizes an inclusive KL divergence. In the next section, we will show that we jointly train a RWS-style encoder $q_\f(z | x)$ as the initial proposal, along with conditional proposals $q_\f(z_b | x, z_{-b})$.
...
%%%%%Hao Edit
% Approximating the gradient presentschicken-and-egg problem; we need samples from the posterior to compute a Monte Carlo estimate of the gradient, but generating these samples is precisely what we are hoping to use learned proposals for in the first place. 
A well-known limitation of importance sampling is that the computed weights have high variance in models with high-dimensional and/or correlated variables such as models with a hierarchical structure. When the weighted samples poorly approximate the posterior, using the gradient estimators in Equations~\ref{eq:grad-self-normalized} and~\ref{eq:grad-theta} for training can be slow.
...
To overcome this problem, SMC samplers interleave application of the transition kernel with a \emph{resampling} step (see Appendix~\ref{appendix:resample-algo} for Algorithm). This step generates a new set of samples by selecting current samples with replacement, with probability proportional to their weight.
...
The resampling reduces the variance of the importance weights at the expense of also reducing the diversity of the sample set; high-weight samples are selected frequently, whereas low-weight samples are selected infrequently or not at all.
...
In our experiments we will consider models with high-dimensional correlated variables, and this scheme greatly increase the tractability of the gradient estimation problem. 
...
% Work by Hoffman~\cite{hoffman2017learning} applies Hamiltonian Monte Carlo to samples that are generated from the encoder, which serves to improve the gradient estimate w.r.t.~$\theta$ (Equation~\ref{eq:grad-theta}), while learning the inference network using a standard reparameterized ELBO objective. 
Work by \citet{hoffman2017learning} applies Hamiltonian Monte Carlo (HMC) to improve samples from the encoder in order to obtain better gradient estimate w.r.t. the parameters of the generative model (Equation~\ref{eq:grad-theta}).
While their inference network is 
trained by maximizing a standard reparameterized ELBO 
...
\citet{wang2018meta} develop a meta-learning approach to learn Gibbs block conditionals. This work assumes a setup in which it is possible to sample $x, z \sim p(x, z)$ from the true generative model $p(x,z)$, which means gradients can be estimated using a sleep-phase Monte Carlo estimator. This circumvents the need for self-normalized estimators of the form in Equation~\ref{eq:grad-self-normalized}, which are necessary when we additionally wish to learn the generative model. Similar to our work, the approach by \citet{wang2018meta} minimizes the inclusive KL, but uses the learned conditionals to directly define an (approximate) MCMC sampler, rather than using them as proposals in an SMC sampler. This work also has a somewhat different focus from ours, in that it primarily seeks to learn block conditionals that have the potential to generalize to previously unseen graphical models.
\fi
...
We train our APG samplers on 20,000 GMM instances with $I = 3$ clusters and $N = 60$ data points with fixed hyperparameters ($\mu_0 = 0$, $\nu_0 = 0.2$, $\alpha_0 = 2$, $\beta_0 = 2$). We use $20$ GMM instances per batch, $K=5$ sweeps, $L=10$ particles, and $\mathrm{lr} = 2.5\times10^{-4}$ learning rate for $2\times10^5$ iterations.
\begin{figure}[t!]
\centering
\includegraphics[width=\columnwidth]{figures/convergence_gmm_v2.pdf}
  \caption{$\log p(x, z)$ as a function of number of sweeps $K$ for a GMM instance. The APG sampler and Gibbs sampler have a sampling budget of $L=100$ samples per sweep, while BPS has a sampling budgets of $10,000$ per sweep. HMC-RWS has a budget of $100$ samples per leapfrog step ($LS$ leapfrog steps are performed per sweep). To achive results comparable to AGP and Gibbs samplers, HMC-RWS needs a $10$ times larger sampling budget while BPS under-performs even with a $100$ times larger sampling budget.}
...
Figure~\ref{fig:samples-gmm} shows evolution of a single sample from variational distribution in one GMM with $N=100$ data points. The sample is initialized using RWS encoder and updated with APG sweeps. Even when using a parameterization that employs neural sufficient statistics, the RWS encoder fails to propose reasonable clusters, whereas the APG sampler typically converges within 12 sweeps across GMM instances with a range of variables.

Furthermore, we compare the APG sampler with other iterative inference methods: (1) Bootstrapped population sampling (BPS) where we propose $\mu, \tau$ from the prior $p(\mu, \tau)$ instead of $q_\f(\mu, \tau | x, z)$; (2) True Gibbs sampling; (3) Hamiltonian Monte Carlo (HMC) method for continuous variables $\mu, \tau$ by marginalizing the discrete variables $c$, which is updated by Gibbs update $p(c | x, \mu, \tau)$. Figure~\ref{fig:convergence-gmm} shows that APG sampler outperforms the HMC with 10 times as many computation budget and BPS with 100 times as many computation budget. Note that APG sampler converges slightly faster than the Gibbs, because resampling in APG helps to select higher quality proposals.
% \begin{table}[]
%     \centering
%     \begin{tabular}{c|c}
%          &  \\
...
\includegraphics[width=\columnwidth]{figures/budget_gmm.pdf}
  \caption{$\log p(x, z)$ (left) and ESS (right) as a function of number of sweeps for APG samplers with different block update strategies and same overall sampling budget $K \cdot L = 1000$. A higher ESS can be is achived by performing more sweeps with fewer samples.}
...
For the GMM the test corpus contains $20,000$ instances, each of which has $N=100$ data points; In the DMM the test corpus contains $20,000$ instances with $N=600$ data points per instance; In the bouncing MNIST experiment the test corpus contains $10,000$ instances with $T=100$ time steps and $D=5$ digits.}
...
\begin{tabularx}{\textwidth}{cccccccccc}
    ...
    \midrule
    GMM ($\times10^2$)& $-7.651$ & $-6.688$ & $-6.594$ & $-5.420$ & $-4.673$ & $-4.816$ & $-4.556$ & $-4.469$ & $-4.573$ \\
    DMM ($\times10^3$) & $-3.172$ & $-4.204$ & $-2.185$ & $-2.184$ & $-2.184$ & $-2.050$ & $-2.040$ & $-2.035$ & -- \\
    BC-MNIST ($\times10^5$) & $-1.673$ & $-1.884$ & $-1.538$ & $-1.405$ & $-1.303$ & $-0.706$ & $-0.652$ & $-0.620$ & --\\
    \bottomrule
\end{tabularx}
...
As a mean of comparing the performance of APG samplers for varying numbers of sweeps $K$, we perform an experiment in which the computation budget is fixed at $K \cdot L = 1000$ samples. Because the marginal $q_\f(z^k | x)$ is intractable, it is difficult to compute an lower bound or upper bound at each sweep. Here we compute the log joint distribution $\log p_\q(\x, z)$. We can see that the APG sampler converges much faster even compared with other methods that use more computation budget. In addition to log joint $\log p_\theta(x, z)$, we also report the effective sample size to assess proposal quality 
\begin{align*}
% \label{ess-eq}
    \frac{\text{ESS}}{L} 
    = 
...
\end{align*}
Here $\mu_i$ is center of the $i$th ring. Given a cluster assignment $c_n$ and an embedding $\alpha_n$ which goes into a feedforward decoder $g_\q$, we sample a data point $x_n$ with 2D Gaussian noise.

We train our model on 20,000 DMM instances with $N = 200$ data points and $I = 4$ clusters with fixed hyperparameters ($\sigma_0 = 3$, $\sigma^2_{\epsilon} = 0.1$). We use $20$ datasets per batch, $K=8$ sweeps, $L=10$ particles, and $\mathrm{lr} = 10^{-4}$ learning rate for $2\times10^5$ iterations (see Appendix \ref{appendix:architecture} for architecture).
...
\begin{table}[t!]
    \centering
    \caption{Mean squared error (MSE) between instances of test corpora (for different number of digits $D$ and time steps $T$, each containing 5000 video instances) and their reconstructions from the generative model. The inference model used to infer the latent variables was trained on instances with $T=10$ time steps. The reconstructions are then generated based on the inferred latent variables. APG sampler achive lower MSE compared to RWS and HMC-RWS and scales to $T=100$ time steps at test time.}
...
We train our model on 60000 bouncing MNIST instances, each of which contains $D=3$ digits and $T=10$ frame images. We use $5$ sequences per batch, $K=5$ sweeps, $L=10$ particles, $\mathrm{lr} = 10^{-4}$ learning rate for $1.2\times10^7$ iterations (see Appendix \ref{appendix:architecture} for architecture).
