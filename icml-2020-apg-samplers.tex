...
\end{align*}
with initial weight $w^1 = \gamma^1(z^1) / q^1(z^1)$.
This construction ensures that, at step $k$ in the sequence, we have a weight $w^k$ relative to the intermediate  density $\gamma^k(z^{1:k})$ of the form (see Appendix~\ref{appendix:sis-weight} for a full derivation)
\begin{align*}
    w^k
    =
    \frac{\gamma^k(z^{1:k})}
...
\begin{align}
    ...
    .
\end{align}
This construction is valid for \emph{any} proposal kernel $q(z^k \mid z^{k-1})$ and reverse kernel $r(z^{k-1} \mid z^{k})$. The \emph{optimal} reverse kernel for a proposal is
\begin{align*}
    r(z^{k-1} \mid  z^k)
    =   
    \frac{p_\q(x,z^{k-1})}
...
In APG samplers, we will employ resampling after each block update $z_b\sim q_\phi(z_b | x, z_{-b})$. Because the incoming weights are now equal at each block update, we can compute gradient estimates using incremental weights $v$ of the form
\begin{align}
    v
    = 
