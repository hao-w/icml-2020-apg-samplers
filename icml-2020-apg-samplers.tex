...
These models are trained by minimizing the stochastic lower bound (ELBO) in Equation~\ref{eq:elbo} on the log marginal likelihood which is equivalent to minimizing the exclusive KL divergence $\mathrm{KL}(q_\f(z | x) || p_\q(z | x))$. 
\begin{align}
    \label{eq:elbo}
    \mathcal{L} (\f)
    &
...
%%%%%Hao Edit
A well-known limitation of importance sampling is that the computed weights have high variance for high-dimensional and/or correlated variables such as models with hierarchical structure. When the weighted samples poorly approximate the posterior the gradient estimates in Equations~\ref{eq:grad-self-normalized} and~\ref{eq:grad-theta} will have high variance leading to slow convergence rates.
...
SMC methods \cite{doucet2001sequential} reduce the variance of importance weights by decomposing a high-dimensional sampling problem into a sequence of lower-dimensional problems. To achive this they combine two basic ideas. First, sequential importance sampling, which decomposes a proposal for a sequence of variables into a sequence of conditional proposals, and (2) resampling, which selects partial proposals with probability proportional to their weights in order to improve the overall sample quality. 
